...
**`build-config-editor-linux` pins `ubuntu-22.04`** (not `ubuntu-latest`): AppImage/deb link against the runner's glibc, so the oldest supported runner maximizes user-distro compatibility. Builds `--bundles appimage,deb` only (no rpm — runner has no `rpmbuild`). Needs `libwebkit2gtk-4.1-dev` + `libfuse2`, with `APPIMAGE_EXTRACT_AND_RUN=1` for the FUSE-less runner. Also needs `xdg-utils` (`/usr/bin/xdg-open`) for the AppImage bundler — preinstalled on the amd64 runner image but **not** the arm64 one, so it must be in the apt list or the arm64 leg fails with `xdg-open binary not found`.
