...
- **Runtime version**: `code.py` reads from a `VERSION.txt` file (not hardcoded). The file is generated by `git describe --tags --always` during build/deploy. Falls back to `"dev"` if the file is missing.
- `VERSION.txt` is written by both distribution paths: `deploy.sh` and `ci.yml`
...
- **CI workflow** (`.github/workflows/ci.yml`): Runs on push to any branch AND on `v*` tag push
  ...
  - Writes `VERSION.txt` file into firmware zip from git tag/describe
...
- **Firmware VERSION patching**: Release workflow patches `/VERSION.txt` inside the firmware zip with the clean tag, since the CI-built VERSION.txt contains a `git describe` string.
