...
- [ ] Pages / banks
- [ ] `cc_inc`/`cc_dec` STEP mode: optional snap of incoming CC values to the `cc_step` grid from `cc_min` (today the value is taken as-is and the step is a stride; SLOT mode already snaps). Opt-in per button. See docs/plans/2026-09-08-issue-11-cc-inc-dec-design.md.
