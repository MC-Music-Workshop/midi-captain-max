...
- **CI workflow** (`.github/workflows/ci.yml`): Runs on push to any branch AND on `v*` tag push
  ...
  - **Don't retry `compression-level: 0` on the firmware-zip upload.** Empirically tested 2026-04-21 (commits `51d08ba` → `ed73bef`): setting `compression-level: 0` on `actions/upload-artifact@v7` for `firmware-zip` made the artifact ~10% *larger* (82 KB → 90 KB) while saving only ~3 s. Default wins because the GitHub wrapper's zip metadata (central directory, headers) *does* compress even when the payload doesn't. `archive: false` would drop the wrapper entirely but is blocked by upstream bug `actions/upload-artifact#785` (ignores `name:` input).
