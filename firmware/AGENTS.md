...
- `_process_midi_msg` is source-agnostic; CC RX button matching lives in `core/midi_rx.py` (`find_cc_rx_action`, unit-tested in `tests/test_midi_rx.py`) — side effects stay in `code.py`. NoteOn/Off and PC are also handled.
- **CC RX matching rules** (#155/#163): select-mode buttons activate only on an exact `cc_on` match; several select buttons may share one CC number and differ by `cc_on` (Helix snapshots). Once a select button claims a CC, non-select buttons on that CC are shielded from RX. Non-select buttons match `cc_on`/`cc_off` exactly, with a `>63` fallback for other values — so with a custom `cc_on=10`, an incoming 80 still turns the button ON via the fallback (deliberate: generic hosts sending 0/127 keep working), and `cc_off > 63` turns OFF on exact match where pre-#155 firmware turned ON. `cc_on == cc_off` leaves the off-branch dead (`cc_on` wins; boot-time `[CONFIG WARN]`).
